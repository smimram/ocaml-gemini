...
let rec loop client model history =
  ...
  | input ->
      ...
      if line = "" then
        ...
        let user_message = { C.role = User; parts = [ Text line ] } in
        ...
        | Ok response ->
            ...
            let model_message =
              { C.role = Model; parts = [ Text response.text ] }
            in
...
let () =
  ...
  let set_base_url s = base_url := Some s in
  let usage = "ocaml-gemini-chat [--model MODEL] [--api-key KEY] [--base-url URL]" in
  let specs =
    [ ("--model", Arg.Set_string model, "Gemini model to use");
      ("--api-key", Arg.String set_api_key, "Gemini API key");
      ("--base-url", Arg.String set_base_url, "Gemini API base URL")
    ]
  in
  ...
  let client = C.create ?base_url:!base_url ?api_key:!api_key () in
  loop client !model []
